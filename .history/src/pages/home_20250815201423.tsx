      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {isLoading ? (
            ...
            <div>
              {/* Grid layout for featured coffee bags */}
                ...
                <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
                    ...
                    <div className="p-6 flex flex-col justify-between">
                      <div>
                        <h4 className="font-bold text-xl text-gray-900 mb-2">Premium Coffee Bag</h4>
                        <p className="text-gray-600 text-sm mb-3">
                          Premium single-origin coffee beans in an elegant bag. Perfect for coffee enthusiasts seeking exceptional quality and rich flavor profiles.
                        </p>
                        <div className="text-sm text-gray-500 mb-3">
                          <div><strong>Origin:</strong> Colombia</div>
                          <div><strong>Process:</strong> Washed</div>
                          <div><strong>Rating:</strong> ⭐ 4.8</div>
                        </div>
                        <div className="text-lg font-bold text-gray-900 mb-1">$24.99</div>
                        <div className="text-sm text-gray-500">0.012 ETH | 24.99 USDC</div>
                      </div>
                      <Button 
                        onClick={() => {
                          const { addItem } = require('../hooks/use-cart').useCart();
                          const { toast } = require('../hooks/use-toast').useToast();
                          addItem({
                            productId: 'featured-coffee-bag-1',
                            quantity: 1,
                            product: {
                          ...
                          toast({
                            title: "Added to Cart",
                            description: "Premium Coffee Bag has been added to your cart",
                          });
                        }}
                        className="w-full mt-4 bg-coffee-brown hover:bg-coffee-brown/90 text-white"
                      >
                        Add to Cart
                      </Button>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
                    <div className="aspect-square overflow-hidden">
                      <img 
                        ...
                        alt="Artisan Coffee Blend"
                        className="w-full h-full object-cover hover:scale-105 transition-transform duration-300"
                      />
                    </div>
                    <div className="p-6 flex flex-col justify-between">
                      <div>
                        <h4 className="font-bold text-xl text-gray-900 mb-2">Artisan Coffee Blend</h4>
                        <p className="text-gray-600 text-sm mb-3">
                          Carefully crafted artisan blend combining beans from multiple origins for a complex, balanced flavor experience.
                        </p>
                        <div className="text-sm text-gray-500 mb-3">
                          <div><strong>Origin:</strong> Ethiopia & Brazil</div>
                          <div><strong>Process:</strong> Natural & Washed</div>
                          <div><strong>Rating:</strong> ⭐ 4.6</div>
                        </div>
                        <div className="text-lg font-bold text-gray-900 mb-1">$19.99</div>
                        <div className="text-sm text-gray-500">0.009 ETH | 19.99 USDC</div>
                      </div>
                      <Button 
                        onClick={() => {
                          const { addItem } = require('../hooks/use-cart').useCart();
                          const { toast } = require('../hooks/use-toast').useToast();
                          addItem({
                            productId: 'featured-coffee-bag-2',
                            quantity: 1,
                            product: {
                          ...
                          toast({
                            title: "Added to Cart",
                            description: "Artisan Coffee Blend has been added to your cart",
                          });
                        }}
                        className="w-full mt-4 bg-coffee-brown hover:bg-coffee-brown/90 text-white"
                      >
